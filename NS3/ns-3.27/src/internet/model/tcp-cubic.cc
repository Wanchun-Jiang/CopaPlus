      ...
      if (m_cWndCnt >= cnt)
        {
          ...
          m_cWndCnt -= cnt;
          NS_LOG_INFO (this << " In CongAvoid, updated to cwnd " << tcb->m_cWnd << " i.e., " << tcb->GetCwndInSegments ());
        }
      else
        {
...
TcpCubic::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
  ...
  uint32_t segCwnd = tcb->GetCwndInSegments ();
  NS_LOG_INFO ( this << " Loss at cWnd=" << segCwnd << " segments in flight=" << bytesInFlight / tcb->m_segmentSize);

...
//                   MakeEnumChecker (CLOSED, "Closed"))
	.AddAttribute ("initialssThresh", "initial size of ssThresh",
	                   UintegerValue (128),
	                   MakeUintegerAccessor (&TcpSocketBase::SetInitialSSThresh,
                               &TcpSocketBase::GetInitialSSThresh),
    ...
    .AddAttribute ("Sack", "Enable or disable Sack option",
                   ...
                   MakeBooleanChecker ())
	.AddAttribute ("Pdcc", "Enable or disable PDCC option",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_pdccEnabled),
                   MakeBooleanChecker ())//^_^
...
TcpSocketBase::DupAck ()
{
  ...
  std::cerr<<this<< " lhyDebug-dupack: "<< m_dupAckCount<< " Threshold "<< m_retxThresh << " Sack " << m_sackEnabled << " State "<< m_tcb->m_congState<< " m_cWnd " << m_tcb->m_cWnd/m_tcb->m_segmentSize << " m_highRxAckMark " << m_highRxAckMark << " m_recover "<< m_recover << std::endl;
...
TcpSocketBase::ProcessAck (const SequenceNumber32 &ackNumber, bool scoreboardUpdated)
{
  ...
  //std::cerr<<"lhyDebug-dupack: "<< m_dupAckCount<< " Threshold "<< m_retxThresh << " scoreboardUpdated " << scoreboardUpdated << " Sack " << m_sackEnabled << " ack " << ackNumber << " next "<< m_tcb->m_nextTxSequence << std::endl;
          ...
          if (!m_sackEnabled && exitedFastRecovery)
            {
              ...
              m_congestionControl->ExitRecovery (m_tcb, segsAcked);
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (pacing_rate == 0.0||m_tcb->m_congState!=TcpSocketState::CA_OPEN) {
  ...
  if (!m.IsZero ())
    {
      ...
      // RFC 6298, clause 2.4
      m_rto = Max (m_rtt->GetEstimate () + Max (m_clockGranularity, m_rtt->GetVariation () * 4), m_minRto);
      //if(m_congestionControl->GetName()) m_lastRtt=m;
      m_congestionControl->SetrttInst(m);
      m_lastRtt = m_rtt->GetEstimate ();

      //NS_LOG_UNCOND(this<<"sRTT "<<m_lastRtt<<" rttI "<<m_congestionControl->GetrttInst());
...
TcpSocketBase::ReTxTimeout ()
{
  ...
  // Cwnd set to 1 MSS
  if(!m_pdccEnabled)m_tcb->m_cWnd = m_tcb->m_segmentSize;//^_^ Reset Cwnd when timeout

...
Time TcpCongestionOps::GetrttInst () const{
	return m_rttInst ;
}
void TcpCongestionOps::SetrttInst (Time para){
	m_rttInst = para;
}
  ...
  if (segmentsAcked >= 1)
    {
      ...
      NS_LOG_INFO (this <<" "<<Simulator::Now ().GetSeconds ()<<" In SlowStart, updated to cwnd " << tcb->m_cWnd << " ssthresh "
    		  << tcb->m_ssThresh<<" initial "<<tcb->m_initialSsThresh);
  ...
  if (segmentsAcked > 0)
    {
      ...
      NS_LOG_INFO (this <<" "<<Simulator::Now ().GetSeconds ()<<" In CongAvoid, updated to cwnd " << tcb->m_cWnd <<
                   " ssthresh " << tcb->m_ssThresh<<" initial "<<tcb->m_initialSsThresh);
    }
}
...
{
  NS_LOG_FUNCTION (this << tcb << segmentsAcked<<Simulator::Now ().GetSeconds());
  //NS_LOG_UNCOND (this << " "<<Simulator::Now ().GetSeconds());
  if((uint32_t)tcb->m_ssThresh>=30000){tcb->m_ssThresh=30000;}

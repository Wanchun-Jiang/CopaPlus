...
PointToPointNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PointToPointNetDevice")
    ...
    /* .AddAttribute ("isRandomChange", "the mode of rate change",
		   	   	      BooleanValue (false),
					  MakeBooleanAccessor (&PointToPointNetDevice::isRandomChange),
					  MakeBooleanChecker ())
	.AddAttribute ("isLossChange", "the mode of loss rate change",
		   	   	      BooleanValue (false),
					  MakeBooleanAccessor (&PointToPointNetDevice::isLossChange),
					  MakeBooleanChecker ())
	.AddAttribute ("RandomInterval", 
                   "The time update random environment",
                   TimeValue (Seconds (0x3f3f3f3f)),
                   MakeTimeAccessor (&PointToPointNetDevice::m_interval),
                   MakeTimeChecker ()) */
...
PointToPointNetDevice::Attach (Ptr<PointToPointChannel> ch)
{
  ...
  /* if(isRandomChange){
	if(!m_RandomRate){
	  m_RandomRate=CreateObject<UniformRandomVariable> ();
	  //ScheduleRateChange(); 
...
void
PointToPointNetDevice::Setlossrate (double value)
{
	Ptr<RateErrorModel> em = CreateObject<RateErrorModel> ();
	em->SetAttribute ("ErrorRate", DoubleValue (value));
	RateErrorModel::ErrorUnit eunit=RateErrorModel::ERROR_UNIT_PACKET;
	em->SetUnit(eunit);
	SetReceiveErrorModel (PointerValue (em));
}

/* void
PointToPointNetDevice::ScheduleRateChange (void)
{
  if(isRandomChange || isLossChange){
	  if(!m_RandomRate) m_RandomRate=CreateObject<UniformRandomVariable> ();
	  double value = m_RandomRate->GetValue (10,100);//s
	  if(isRandomChange){
	  ...
	  //loss
	  if(isLossChange){
		  value = m_RandomRate->GetValue (0,100)/10000;
		  Ptr<RateErrorModel> em = CreateObject<RateErrorModel> ();
		  em->SetAttribute ("ErrorRate", DoubleValue (value));
		  RateErrorModel::ErrorUnit eunit=RateErrorModel::ERROR_UNIT_PACKET;
		  em->SetUnit(eunit);
	  ...
	  Simulator::Schedule (tNext, &PointToPointNetDevice::ScheduleRateChange, this);
  }
}*/

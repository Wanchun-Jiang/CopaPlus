...
BulkSendApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BulkSendApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
   .AddAttribute ("CCProtocol", "The type of protocol to use.",
						 TypeIdValue (TcpCopa::GetTypeId ()),
						 MakeTypeIdAccessor (&BulkSendApplication::m_CCTpid),
						 MakeTypeIdChecker ())
...
void BulkSendApplication::SetSocket (Ptr<Socket> s){
	m_socket=s;
}
  ...
  if (!m_socket)
    {
      ...
      Ptr<TcpSocketBase> sc= Socket::CreateSocket (GetNode (), m_tid)->GetObject<TcpSocketBase>();

      ObjectFactory congestionAlgorithmFactory;
      congestionAlgorithmFactory.SetTypeId (m_CCTpid);
      Ptr<TcpCongestionOps> algo = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
      sc->SetCongestionControlAlgorithm (algo);
      m_socket=sc;
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO (Simulator::Now ().GetSeconds()<< " "<<this);

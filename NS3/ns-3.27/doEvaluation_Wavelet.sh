...
do
postfix=$loop
./waf --run "periodcCCEvaDumbbellforWavelet --runtime=$endtime --flowNum=$loop --delayRB=20 --BW=48 --Bd=10 --Rc=30 --lamuda=1 --postfix=$postfix --rand_loss=false --rand_BW=true --rand_delay=false --rand_interval=5 --queuesize=320 --cycle=0.5 --CC_mode=5"  > Testdata/Evaluation/"$dir"/output"$postfix".txt 2>&1 \
&& \

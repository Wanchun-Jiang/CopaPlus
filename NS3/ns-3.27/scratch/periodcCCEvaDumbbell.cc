...
std::string fileNameRoot = "Testdata/Evaluation/";
...
main (int argc, char *argv[])
{
  ...
  int CC_mode=1;// 0-newReno 1-PDCC 2-Copa 3-BBR 4-ADC
  ...
  std::string sroot;
  if(iscopaModify){
	  sroot=fileNameRoot+"copaModify";
  }else{
	  sroot=fileNameRoot+"copa";
  }
  ...
  AR1.SetQueue("ns3::DropTailQueue", "MaxPackets", UintegerValue(queuesize));
  ...
  AR2.SetQueue("ns3::DropTailQueue", "MaxPackets", UintegerValue(queuesize));
  ...
  if (true){
  AsciiTraceHelper ascii;
  std::string tfname = fileNameRoot + "PDCC.tr";

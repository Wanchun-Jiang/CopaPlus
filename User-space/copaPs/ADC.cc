...
void AdaptiveCC::update_intersend_time() {
  ...
  // Update the window
    ...
    if (_the_window < target_window) {
      ++ update_dir;
      _the_window += update_amt * theta / (delta * _the_window);
    }
    else {
      -- update_dir;
      _the_window -= update_amt* theta / (delta * _the_window);
  ...
  std::cout << "time= " << cur_time << " window= " << _the_window << " target= " << target_window << " rtt= " << rtt << " min_rtt= " << min_rtt << " delta= " << delta << " update_amt= " << update_amt << " Qd= " << queuing_delay << " theta= " << theta << " nearEmpty? " << tempEmpty << endl;
